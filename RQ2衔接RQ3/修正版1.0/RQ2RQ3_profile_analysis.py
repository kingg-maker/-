...
HEALTH_PATH = r"C:\Users\26301\Documents\Codex\2026-08-07\w\work\raw\新增数据\2011\health_status_and_functioning.dta"
...
def norm_hh_2011(value: object) -> object:
    """Append the missing trailing zero to a 2011 raw household ID."""
    if pd.isna(value):
        return value
...
def main() -> None:
    ...
    db_cols = (
        ["ID"]
        + [f"db022s{i}" for i in range(6, 31)]
        + ["db022s34", "db022s35", "db022s40"]
        + [f"db023_{i}_" for i in range(6, 13)]
    )
    db = pd.read_stata(HEALTH_PATH, convert_categoricals=False, columns=db_cols)
    db["ID"] = db["ID"].map(norm_id_2011).astype("string")
    db = db.drop_duplicates(subset=["ID"])

    ch = pd.read_csv(
        CHARLS_PATH,
        low_memory=False,
        usecols=["ID", "householdID", "wave", "family_size", "hchild",
                 "fcamt", "adlab_c", "iadl"],
        dtype={"ID": "string", "householdID": "string"},
    )
    ch = ch[ch["wave"] == 1].drop_duplicates(subset=["ID"])

    hh = pd.read_stata(
        HH_PATH, convert_categoricals=False, columns=["ID", "householdID", "a006"]
    )
    hh["householdID"] = hh["householdID"].map(norm_hh_2011).astype("string")
    hh["live_with_child"] = hh["a006"].isin([7, 8])
    hh_live = hh.groupby("householdID")["live_with_child"].max().reset_index()

    base = (
        base.merge(ch, on="ID", how="left")
        .merge(fam, on="ID", how="left")
        .merge(db, on="ID", how="left")
        .merge(hh_live, on="householdID", how="left")
    )
    hh_cover = int(base["live_with_child"].notna().sum())
    print(f"Household-roster coverage in analysis sample: {hh_cover} / {len(base)} "
          f"({100 * hh_cover / len(base):.1f}%)")
    ...
    base["has_func_diff"] = np.where(
        base["adlab_c"].isna() & base["iadl"].isna(),
        np.nan,
        ((base["adlab_c"].fillna(0) > 0) | (base["iadl"].fillna(0) > 0)).astype(float),
    )
    db_child = base[[f"db022s{i}" for i in range(6, 31)]]
    db_fam = base[["db022s34", "db022s35"]]
    db_resp = base[[f"db022s{i}" for i in range(6, 31)] + ["db022s34", "db022s35", "db022s40"]]
    db_days = base[[f"db023_{i}_" for i in range(6, 13)]]
    base["db_any_response"] = db_resp.notna().any(axis=1)
    base["child_help_any"] = db_child.notna().any(axis=1)
    base["family_help_any"] = db_fam.notna().any(axis=1)
    base["no_one_help"] = base["db022s40"].notna()
    base["child_help_days"] = db_days.sum(axis=1, skipna=True)
    base.loc[base["child_help_days"].eq(0) & db_days.isna().all(axis=1), "child_help_days"] = np.nan
    ...
    for label, col in bin_vars:
        n1m = int(hi[col].notna().sum())
        n0m = int(nh[col].notna().sum())
        a1 = int((hi[col] == 1).sum())
        a0 = int((nh[col] == 1).sum())
        p = prop_pvalue(a1, n1m, a0, n0m)
        rows2.append({
            "类别": "二元变量", "变量": label, "指标": "比例",
            "High": pct_str(a1, n1m), "Non-high": pct_str(a0, n0m),
            "P值": num(p, 3),
        })
    ...
    # Family caregiving (DB module; asked among those with functional difficulties).
    fd_hi = hi[hi["has_func_diff"] == 1]
    fd_nh = nh[nh["has_func_diff"] == 1]
    hi_k = fd_hi[fd_hi["db_any_response"] == 1]
    nh_k = fd_nh[fd_nh["db_any_response"] == 1]
    care_rows = [
        ("功能困难者中：有子女帮助(%)", "child_help_any"),
        ("功能困难者中：子女配偶/孙辈帮助(%)", "family_help_any"),
        ("功能困难者中：无人帮助(%)", "no_one_help"),
    ]
    for label, col in care_rows:
        a1 = int(hi_k[col].sum())
        a0 = int(nh_k[col].sum())
        p = prop_pvalue(a1, len(hi_k), a0, len(nh_k))
        rows2.append({
            "类别": "家庭照护", "变量": label, "指标": "比例",
            "High": pct_str(a1, len(hi_k)), "Non-high": pct_str(a0, len(nh_k)),
            "P值": num(p, 3),
        })
    rows2.append({
        "类别": "家庭照护", "变量": "功能困难者人数(N)",
        "指标": "人数", "High": str(len(fd_hi)), "Non-high": str(len(fd_nh)),
        "P值": "",
    })
    hi_days = hi_k.loc[hi_k["child_help_any"] == 1, "child_help_days"].dropna()
    nh_days = nh_k.loc[nh_k["child_help_any"] == 1, "child_help_days"].dropna()
    rows2.append({
        "类别": "家庭照护", "变量": "子女帮助天数(过去一月, 有子女帮助者, 均值)",
        "指标": "Mean",
        "High": num(hi_days.mean()) if len(hi_days) else "NA",
        "Non-high": num(nh_days.mean()) if len(nh_days) else "NA",
        "P值": num(welch_pvalue(hi_days.to_numpy(float), nh_days.to_numpy(float)), 3)
        if len(hi_days) and len(nh_days) else "",
    })
    rows2.append({
        "类别": "家庭照护", "变量": "子女帮助天数(过去一月, 有子女帮助者, 中位数)",
        "指标": "Median",
        "High": num(hi_days.median()) if len(hi_days) else "NA",
        "Non-high": num(nh_days.median()) if len(nh_days) else "NA",
        "P值": "",
    })
    ...
    n_hi = len(hi)
    n_nh = len(nh)
    hh_cover_pct = 100 * hh_cover / len(base)
    md = []
    ...
    md.append(f"- 与子女同住：按 2011 年家庭名册 `hhmember` 以 `householdID` 合并，任意家庭成员与家庭受访者（FamilyR）关系为子女或子女配偶（`a006`=7/8）。本样本名册可匹配率约 {hh_cover_pct:.0f}%，未覆盖样本按缺失处理，百分比以名册可匹配者为分母。")
    md.append("- `cb053`=1/2（本户或同院相邻）仅作对照口径；家庭模块的子女人名册覆盖不完整，单独用它定义同住会严重低估。")
    md.append("- 子女联系频率：对非同住子女的 `cd004`（`cb053`>=3 的非共居子女），取最频繁者（代码 1=几乎每天，2-3=每周，4-5=每月，6-9=更少/几乎不）。")
    md.append("- 子女照料：来自健康模块 `db022`/`db023`，仅对存在功能困难者提问。`db022s6-s30` 任一非缺失=有子女帮助，`db022s34/s35`=子女配偶/孙辈帮助，`db022s40`=无人帮助；`db023_6_-_12_` 为过去一月各子女帮助天数（加总，问卷最多覆盖 7 个子女槽位）。百分比分母为有功能困难且对 DB 模块有回应者。")
