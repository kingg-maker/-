# ============================================================
# City-level cluster bootstrap for revised RQ1 outcomes.
# Runs after RQ1_revised.R has created revised_data.csv.
# func_cap: 999 reps; cog_cap: 499 reps to keep runtime bounded.
# ============================================================
...
run_city_cluster_boot <- function(data, y, reps, seed = 20260815) {
  ...
  for (r in seq_len(reps)) {
    ...
    if (is.null(agg)) {
      boot_rows[[r]] <- data.frame(att = NA, se = NA)
    } else {
      boot_rows[[r]] <- data.frame(att = agg$overall.att, se = agg$overall.se)
    }
...
boot_func <- run_city_cluster_boot(analysis, "func_cap", reps = 999)
write_csv(boot_func, file.path(res_dir, "rq1_cluster_boot_func_cap_v2.csv"))
