...
p2_data <- es_long %>%
  filter(group == "bhci") %>%
  mutate(label = "Composite health index (BHCI)")
...
pl_long <- pl %>%
  rename(Functional = placebo_func, BHCI = placebo_bhci) %>%
  pivot_longer(cols = c(Functional, BHCI),
               names_to = "label", values_to = "estimate")
obs_lines <- data.frame(
  label = c("Functional", "BHCI"),
...
plot_pl <- function(d, obsrow) {
  ggplot(d, aes(x = estimate)) +
    geom_histogram(bins = 30, fill = "grey75", colour = "grey35",
                   linewidth = 0.2) +
    geom_vline(xintercept = obsrow$obs, colour = "#C8102E",
               linetype = "solid", linewidth = 0.8) +
    geom_vline(xintercept = obsrow$p95, colour = "grey30",
...
pl1 <- plot_pl(pl_long %>% filter(label == "Functional"),
               obs_lines[1, ])
pl2 <- plot_pl(pl_long %>% filter(label == "BHCI"),
               obs_lines[2, ])
